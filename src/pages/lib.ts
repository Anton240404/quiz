...
export function calculateTourResult(pages: Page[]) {
    const correctAnswers = pages.filter((page) => {
        if (page.type === 'SingleAnswerQuestionPage') {
            ...
            return page.selectedAnswer === page.correctAnswer;
        } else if (page.type === 'MultiSelectAnswerQuestionPage' || page.type === 'MultiSelectAnswerAndQuestionImagePage') {
            if (!page.correctAnswers || page.correctAnswers.length === 0) {
                return (!page.selectedAnswers || page.selectedAnswers.length === 0);
            }
            if (!page.selectedAnswers || page.selectedAnswers.length === 0) {
                return false;
            }
            const numCorrectlySelectedByPlayer = page.selectedAnswers.filter(sa => page.correctAnswers.includes(sa)).length;
            return (numCorrectlySelectedByPlayer / page.correctAnswers.length) > 0.5;
